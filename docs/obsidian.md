...
Each tracked todo also gets a generated status link on its original row. A
...
These annotations are generated by Python from durable state, never by the
model. Their checksummed HTML comments identify the exact spans Symphony owns.
They are removed from specification/checklist hashing and model input, so status
refreshes do not request another implementation. Existing source characters,
Waypoint content, BOM, and line endings are preserved. The model still has no
access to the Obsidian directory. Status links remain available when
`manage_checkboxes = false`; only checkbox characters stay manually managed.

Keep your instructions outside the generated comment spans. If one is edited
or incompletely synced, Symphony stops that note's processing and preserves it
instead of guessing what to overwrite. Retain your edits before removing a
damaged generated span. Before each source update the wrapper saves the original
...
5. Writes `_symphony/STATUS.md` and per-repository progress/screenshots under
   ...
   devices. Only the wrapper updates repository metadata, checklist status
   characters, and owned status annotations; the agent never rewrites your
   source notes.

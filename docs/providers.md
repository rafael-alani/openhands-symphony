...
| Codex CLI 0.144.4 | Supported by official `codex login --device-auth`; clean-VM login pending | Implemented; subscription-backed smoke pending | `@agentclientprotocol/codex-acp` 1.1.4 | Agent Server cancel/resume proved with a fake ACP process; provider smoke pending | Implemented; GitHub smoke pending | No OpenAI API key is configured; real ChatGPT-subscription smoke is still required |
| Antigravity CLI 1.1.3 | Official Google OAuth/keyring flow documented; clean-VM login pending | **Disabled by default; not yet verified** | Implemented Symphony bridge over ACP 0.11.0 and official `agy --print` | OpenHands-level continuation only; native conversation resume is not claimed | Implemented but unverified | Headless flags/checksum were spiked without a model turn; no subscription-backed Ubuntu run has passed |
...
The Antigravity 1.1.3 binary, checksum, and headless flags were verified locally on 2026-07-16 without consuming a model turn. This is not evidence that autonomous subscription execution works. The shipped example therefore sets `providers.antigravity.enabled = false`; enable it only after `agentctl auth antigravity`, `agentctl doctor`, and a disposable subscription-backed Ubuntu smoke run all pass.

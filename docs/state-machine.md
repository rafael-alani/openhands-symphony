...
Claiming atomically inserts a lease, moves `queued` to `running`, records the owner/expiry, and starts heartbeat timestamps. The implementation attempt is incremented only after live-issue preflight, provider authentication/health checks, checkout, integrity checks, repository setup, and provider conversation creation/resumption succeed. Setup, preflight, and rejected provider launches therefore do not spend the bounded implementation-attempt budget. Heartbeats renew the lease. A worker cancellation or `/agent pause` interrupts the OpenHands conversation.

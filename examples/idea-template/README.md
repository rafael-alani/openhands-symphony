...
`.symphony/idea.toml` declares the coding provider and a truthful preview command. `preview.start` is an argument array, never a shell command. The app must bind to the declared loopback port and answer at `health_path` within `startup_timeout_seconds`.

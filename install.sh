...
echo "   sudo -iu ${AGENT_USER} agentctl auth antigravity  # only when explicitly enabling it"
echo "   Existing logins are detected before OAuth, so these commands are safe to repeat."
echo "   Antigravity is disabled by default; do not authenticate it unless you explicitly enable and smoke-test it."
echo "   Its optional CLI executable is named 'agy', not 'antigravity'."

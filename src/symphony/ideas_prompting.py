...
Implement the smallest useful solution for the changed wishes and any unresolved pending work identified in previous progress or a supplied project checklist. A changed main brief applies to every section. Revisit question, partial, and not started results using the current specification, even when that section's prose did not change. Work only inside the isolated worktree. Never edit idea/SPEC.md. Do not use gh, push, create issues or pull requests, add plans, perform a self-review, deploy, or otherwise touch GitHub. The wrapper owns validation, preview screenshots, PROGRESS.md, commits, and publication.
